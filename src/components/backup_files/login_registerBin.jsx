...
const LoginAndRegister = ({ setToggleForm }) => {
  ...
  const handleSubmitLogin = () => {
    ...
    /* This line is for testing only (Error Banner) */
    if (passwordLogin !== "" && validateEmailLogin(emailLogin) && emailLogin !== "") {
  ...
  const handleSubmitRegister = () => {
    ...
    /* This line is also for testing only (Error Banner) */
    if (firstNameRegister !== "" && 
        lastNameRegister !== "" && 
        emailRegister !== "" && 
        passwordRegister !== "" && 
  ...
  /* Simple Disable Scroll Logic */
  useEffect(() => {
    document.body.style.overflow = setToggleForm ? "hidden" : "auto";
  ...
  return (
    <>
      {/* LOGIN MODAL */}
      <div id="login-modal" className={`fixed inset-0 z-200 bg-[rgba(5,3,1,0.85)] backdrop-blur-[6px] ${formAnimation ? "flex opacity-100" : "flex"} ${!setToggleForm ? "hidden opacity-0" : ""} opacity-0 items-center justify-center p-5 transition-opacity duration-300`}>
        <div className="flex w-full max-w-215 bg-(--cream) shadow-[0_40px_80px_rgba(0,0,0,0.55)] overflow-hidden relative animate-modalSlideIn max-[900px]:max-w-120" id="login-box">
          ...
          <div className="flex-1 px-12 py-11 overflow-y-auto relative max-h-[90vh] max-[900px]:p-9 max-[900px]:px-7 max-[900px]:max-h-screen max-[480px]:p-7 max-[480px]:px-5">
            <button className="absolute top-5 right-5 bg-transparent border-0 cursor-pointer text-(--warm-gray) p-1.5 transition-colors duration-200 rounded-sm hover:text-(--ink) hover:bg-[rgba(0,0,0,0.06)] " aria-label="Close" onClick={closeLogin}>
            ...
            <div id="form-register" className={`auth-form ${tab === "Register" ? "block" : "hidden"}`}>
              ...
              <button className="w-full p-3.5 bg-(--gold) border-0 cursor-pointer font-['Dm_Sans', serif] text-[13px] font-medium tracking-[0.12em] uppercase text-(--ink) flex items-center justify-center gap-2 mt-3 hover:bg-(--gold-light) hover:-translate-y-0.5 transition-all duration-200 disabled:opacity-60 disabled:cursor-not-allowed disabled:transform-none" id="btn-register" onClick={handleSubmitRegister}>
                <span className={`flex`}>
                  Create Account
                </span>

                <span className={`${!emailExist ? "flex" : "hidden"} hidden`}>
                  <svg className="animate-spin" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="18" height="18"><path d="M21 12a9 9 0 1 1-6.22-8.56"/></svg>
                </span>
              </button>
            ...
            <div id="form-forgot" className={`auth-form ${toggleResetPassword && !backToLogin ? "block" : "hidden"}`}>
              ...
              <button className="w-full py-3.5 bg-(--gold) border-0 cursor-pointer font-['DM_Sans',sans-serif] text-[13px] font-medium tracking-[0.12em] uppercase text-(--ink) flex items-center justify-center gap-2 transition-all duration-200 mt-2 hover:bg-(--gold-light) hover:-translate-y-px" onClick={handleResetPassword}>
                <span className="btn-auth-text">
                  Send Reset Link
                </span>
              </button>

              <div id="forgot-success" className="text-center py-8 flex-col items-center gap-4 hidden">
