  ...
  return (
    <>
      ...
      <section className="h-screen relative flex flex-col justify-end px-15 py-20 bg-(--ink) overflow-hidden max-[900px]:px-6! max-[900px]:py-15!">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_70%_40%,rgba(139,26,26,0.35)_0%,transparent_60%),radial-gradient(ellipse_at_20%_80%,rgba(201,168,76,0.2)_0%,transparent_50%),linear-gradient(180deg,rgba(26,18,9,0.3)_0%,rgba(26,18,9,0.85)_100%)]"></div>
        ...
        <div className="relative flex items-center gap-4">
          <a href="#information" className="inline-block px-9 py-3.5 bg-(--gold) text-(--ink) font-['DM_Sans',sans-serif] text-[13px] font-medium tracking-widest uppercase no-underline transition-colors duration-200 hover:bg-(--gold-light)">
            Explore the City
          </a>
          <a href="#history" className="inline-block px-9 py-3.5 border border-white/30 text-white/80 text-[13px] font-medium tracking-widest uppercase no-underline transition-colors duration-200 hover:border-(--gold) hover:text-(--gold)">
            Our History
          </a>
        </div>
